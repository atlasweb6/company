...
class ControllerModuleSoclLogin extends Controller
{
    ...
    public function index() {
        // 21-12-2015: update Ins, Pin network into db
        $query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "soclall_network` WHERE `network_code` = 'instagram'");
        if (!$query->num_rows)
            $this->db->query("INSERT INTO `" . DB_PREFIX . "soclall_network` VALUES ('instagram','Instagram','instagram.png')");
            
        $query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "soclall_network` WHERE `network_code` = 'pinterest'");
        if (!$query->num_rows)
            $this->db->query("INSERT INTO `" . DB_PREFIX . "soclall_network` VALUES ('pinterest','Pinterest','pinterest.png')");
        // end update db
        
        // 25-12-2015: update Amazon, Ebay networks into db
        $query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "soclall_network` WHERE `network_code` = 'amazon'");
        if (!$query->num_rows)
            $this->db->query("INSERT INTO `" . DB_PREFIX . "soclall_network` VALUES ('amazon','Amazon','amazon.png')");
            
        $query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "soclall_network` WHERE `network_code` = 'ebay'");
        if (!$query->num_rows)
            $this->db->query("INSERT INTO `" . DB_PREFIX . "soclall_network` VALUES ('ebay','Ebay','ebay.png')");
        // end update db
